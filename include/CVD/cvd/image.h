...
static inline void SwapImage(Image<TYPE> &img1, Image<TYPE> &img2)
{
	Image<TYPE> imgTmp = img1;
	img1 = img2;
	img2 = imgTmp;
}

template<typename TYPE>
static inline void LoadImage(CVD::Image<TYPE> &img, CVD::Image<TYPE> &imgTmp, const std::string &fileName)
{
	CVD::img_load(imgTmp, fileName);
	if(imgTmp.size().x % 4 == 0)
		img = imgTmp;
	else
	{
		const int width = (imgTmp.size().x + 3) & (~3), height = imgTmp.size().y;
		img.resize(CVD::ImageRef(width, height));
		img.zero();
		for(int y = 0; y < height; ++y)
			memcpy(img[y], imgTmp[y], sizeof(TYPE) * imgTmp.row_stride());
	}
}

template<typename TYPE>
static inline void SaveImageB(const Image<TYPE> &img, FILE *fp)
{
	const ImageRef size = img.size();
	fwrite(&size, sizeof(ImageRef), 1, fp);
//#if _DEBUG
	if(size.x == 0 || size.y == 0)
		return;
//#endif
	fwrite(img.data(), sizeof(TYPE), img.totalsize(), fp);
}

template<typename TYPE>
static inline void SaveImageB(const Image<TYPE> &img, const std::string &fileName)
{
...
template<typename TYPE>
static inline void LoadImageB(Image<TYPE> &img, FILE *fp)
{
	ImageRef size;
	fread(&size, sizeof(ImageRef), 1, fp);
//#if _DEBUG
	if(size.x == 0 || size.y == 0)
		return;
//#endif
	img.resize(size);
	fread(img.data(), sizeof(TYPE), img.totalsize(), fp);
}

template<typename TYPE>
static inline void LoadImageB(Image<TYPE> &img, const std::string &fileName)
{
...
template<typename TYPE>
static inline void CopyImage(const Image<TYPE> &imgSrc, Image<TYPE> &imgDst)
{
//#if _DEBUG
...
static inline void CopyImage(const Image<TYPE_SRC> &imgSrc, Image<TYPE_DST> &imgDst)
{
//#if _DEBUG
	if(imgSrc.size().x == 0 || imgSrc.size().y == 0)
		return;
